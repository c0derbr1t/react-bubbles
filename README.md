...
- [ ] Explain what a token is used for.
      ...
      - A token is used to authenticate a user. It is used in PrivateRoutes to ensure that only an authenticated user can view the pages.

- [ ] What steps can you take in your web apps to keep your data secure?

      - Ensure only valid users can view certain information. Keep the token secret so It can’t be used by those that don’t have access to it.

- [ ] Describe how web servers work.

      - The hardware web-servers are the computers that the web-server programs run on. You send a request to the server and it sends information back.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

      - Crud is Create, Read, Update, and Delete. From the lesson this week we learned Update(.put) and Delete(.delete). Create is often .post (like adding a record), and Read is done with .get to GET information.
